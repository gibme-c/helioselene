#!/usr/bin/env python3
"""
Independent validation of helioselene test vectors using pure Python + ecpy.
...
Usage:
    pip install ecpy
    python validate_test_vectors.py test_vectors/helioselene_test_vectors.json
...
def validate_wei25519(t: TestRunner, data: dict):
    ...
    for v in data.get("x_to_selene_scalar", []):
        label = v["label"]
        result_hex = v["result"]
        if result_hex is None:
            t.ok(f"x_to_selene_scalar/{label} (rejected)")
        else:
            # The conversion formula is: x_wei25519 -> selene scalar
            # This requires knowing the isomorphism, defer to SageMath for full validation
            t.skip(f"x_to_selene_scalar/{label}", "conversion formula defer to SageMath")
...
def main():
    ...
    Q = from_le_hex(params["helios_order"])      # Helios scalar field = Selene base field
    P = from_le_hex(params["selene_order"])       # Selene scalar field = Helios base field
    A = params["curve_a"]                         # curve coefficient a (both curves)
    helios_b = from_le_hex(params["helios_b"])
    selene_b = from_le_hex(params["selene_b"])

    print(f"Helios: y^2 = x^3 + {A}x + b over Fp")
    print(f"  Fp = 0x{P:064x}")
    print(f"  b  = 0x{helios_b:064x}")
    print(f"  order Q = 0x{Q:064x}")
    print(f"Selene: y^2 = x^3 + {A}x + b over Fq")
    print(f"  Fq = 0x{Q:064x}")
    print(f"  b  = 0x{selene_b:064x}")
    print(f"  order P = 0x{P:064x}")

    t = TestRunner()

    # Decode generators for ecpy curve construction
    helios_gen_hex = data["helios_point"]["generator"]
    helios_gen_xy = decode_point_bytes(helios_gen_hex, P, A, helios_b)
    assert helios_gen_xy is not None, "Failed to decode Helios generator"

    selene_gen_hex = data["selene_point"]["generator"]
    selene_gen_xy = decode_point_bytes(selene_gen_hex, Q, A, selene_b)
    assert selene_gen_xy is not None, "Failed to decode Selene generator"

    # Scalars
    validate_scalar_section(t, data["helios_scalar"], "helios", Q)
    validate_scalar_section(t, data["selene_scalar"], "selene", P)

    # Points
    validate_point_section(t, data["helios_point"], "helios", P, A, helios_b, Q, helios_gen_xy)
    validate_point_section(t, data["selene_point"], "selene", Q, A, selene_b, P, selene_gen_xy)
    ...
    # Divisors
    validate_divisor_section(t, data["helios_divisor"], "helios", P, A, helios_b, Q, helios_gen_xy)
    validate_divisor_section(t, data["selene_divisor"], "selene", Q, A, selene_b, P, selene_gen_xy)
