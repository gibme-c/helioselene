...
The precomputed data only needs to be regenerated if the auxiliary curve changes (which would break backwards compatibility with all downstream proofs). The process uses the `helioselene-gen-ecfft` tool — see the project README for the exact commands and auxiliary curve parameters.
